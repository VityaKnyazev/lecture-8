...
*/
Insert Into categories (name, description) values ('фрукты', 'Фрукт — сочный съедобный плод растения. Пример: яблоко, груша и т.д.');
...
SELECT goods.name as product, goods.sort, categories.name as category, storehouse.quantity as quantity, storehouse.unit as unit, storehouse.price as price FROM goods INNER JOIN categories ON goods.category_id=categories.id AND categories.name = 'фрукты' INNER JOIN storehouse ON storehouse.good_id = goods.id ORDER BY product LIMIT 4 OFFSET 0;
SELECT name as product, sort FROM goods WHERE name like '%ый' OR name like '%а' order by name asc;
UPDATE producers SET country = 'Польша';
UPDATE goods SET sort='Американка' WHERE id = 6;
